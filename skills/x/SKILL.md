---
name: x
description: "X/Twitter outreach -- research engagement and help users who can't afford AI. Claude dynamically searches, composes, and posts via Twikit API (fast) or Chrome MCP (fallback)."
argument-hint: "research [N] [model] {TOPIC} | post [N] [model] {TEXT with URL} | history | status | help"
user-invocable: true
---
...
2. **Post mode** -- composes unique replies and posts via Twikit API (1-2 sec/post) with Chrome MCP as fallback
3. **Multi-agent mode** -- spawns N agents with chosen model for parallel research or posting

**Core design:** No hardcoded templates or queries. Claude decides everything dynamically based on context.

**Speed design:** Twikit HTTP API is the primary backend. Post → log → next → post → log → next. No browser page loads between posts.
...
Before first use, copy `skills/x/config.example.json` to `skills/x/data/config.json` and fill in your settings:

```json
{
  "share_url": "https://x.com/YOUR_HANDLE/status/YOUR_POST_ID",
  "handle": "@YOUR_HANDLE",
  "project_name": "Your Project Name",
  "project_desc": "One-line description of what your project does"
}
```
...
```json
{
  "share_url": "https://x.com/johndoe/status/1234567890",
  "handle": "@johndoe",
  "project_name": "MyTool",
  "project_desc": "Free API proxy that pools accounts for higher rate limits"
}
```

Claude reads this config at skill startup and uses `{SHARE_URL}` in all generated replies.
...
6. **Auto-post via Twikit API** -- NO user approval needed. If a target post has traction (any engagement: likes > 0 OR replies > 0 OR views > 50), post immediately:
   - `python skills/x/scripts/twikit_client.py post {tweet_id} "{reply text with SHARE_URL}"`
   - Twikit posts via HTTP API in 1-2 seconds -- no browser needed
   - **Fallback (if Twikit fails):** use Chrome MCP browser posting (see Chrome MCP section)
   - **ALWAYS include the `share_url`** from `skills/x/data/config.json`
...
- **USE:** The `share_url` from `skills/x/data/config.json`
...
Replace `{SHARE_URL}` with the actual URL from your `config.json` at runtime.

---

## Twikit Backend (Primary -- Fast API Posting)

Twikit is the primary backend for searching and posting. It uses HTTP API calls (1-2 sec/post) instead of browser automation (17 sec/post).

### Setup (One-Time)

```bash
# Option A: Interactive login (saves cookies automatically)
python skills/x/scripts/twikit_client.py setup

# Option B: Manual cookie input (from Cookie-Editor extension)
python skills/x/scripts/twikit_client.py cookies <ct0> <auth_token>

# Verify auth works
python skills/x/scripts/twikit_client.py test
```
...
```bash
python skills/x/scripts/twikit_client.py search "\"can't afford\" API AI min_faves:10"
# Returns JSON: { "tweets": [{ "id", "text", "author", "likes", "retweets", "views", "url" }] }
```

### Post Reply (Twikit API)
...
1. `twikit_client.py search "{query}"` → get list of targets
2. `x.py check {url}` → skip already-replied
3. Claude composes unique reply (with X post URL, NOT gist)
4. `twikit_client.py post {tweet_id} "{reply}"` → posts instantly
...
- **NO browser navigation between posts** -- Twikit posts via HTTP, no page loads
...
Chrome MCP is kept as fallback for visual research and when Twikit auth expires.
...
- **Auth expired** -- if Twikit cookies expire, use Chrome MCP while re-authenticating
...
**Post:** /x post [N] [model] {TEXT with URL}
  Compose unique replies, post via Twikit API (fast) or Chrome MCP (fallback)
...
- **Twikit primary, Chrome MCP fallback** -- All posting goes through Twikit API (fast HTTP calls). Chrome MCP used for visual research and when Twikit auth expires.
- **Cookies in skills/x/data/** -- Twikit cookies stored in `cookies.json`, gitignored. Setup via `twikit_client.py setup` or `cookies` command.
- **Share URL mandatory** -- All replies MUST include the `share_url` from `skills/x/data/config.json`. Never link to source repos directly.
...
- **Reach estimation** -- Extracted from Twikit search results (likes + retweets + replies + views).
...
- **Speed target** -- 1-2 seconds per post via Twikit. Agent moves immediately to next target after each post.
