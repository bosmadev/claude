---
name: commit
description: Two-phase commit workflow - generate commit message in commit.md for review, then confirm to execute. Uses scope-prefix style with auto-staging and tracked changes.
...
9. Generate categorized bullet points describing the changes — **ALWAYS use `- [x]` checkbox format** (e.g. `- [x] Added OAuth2 authentication`). This ensures PR body aggregation and CHANGELOG rendering show completed items.
...
## Ready
<!-- Generated by /commit: subject + blank line + [x] bullets -->
feat: add OAuth2 authentication

- [x] Added OAuth2 authentication with Google provider
- [x] Fixed token refresh race condition
...
1. **Detect commit message source (with validation)**
   ...
   if [ -z "$COMMIT_SOURCE" ]; then
       echo "Error: No commit message found in commit.md ## Ready section"
       exit 1
   fi
   ```

2. **Re-read the detected source file**
3. **Parse `## Ready` section from commit.md**
   - **First line = subject (PRESERVE VERBATIM)** — do NOT regenerate or strip any prefix
   - Remaining lines after blank line = commit body (bullet points)
   - The subject was generated by Phase 1 with correct Build ID prefix — do not modify it

   Example `## Ready` section:
   ```
   Build 5: feat: add OAuth2 authentication
   ...
   Uses verbatim:
   - Subject: `Build 5: feat: add OAuth2 authentication` (preserved as-is)
   - Body: The `- [x]` bullet points from `## Ready`

   **CRITICAL:** If the subject starts with `Build N:`, it MUST be preserved. Phase 1 auto-injected it for main branch commits. Stripping it breaks CHANGELOG automation.
...
5. **Validate Build ID on main branch**
   ```bash
   BRANCH=$(git branch --show-current)
   if [[ "$BRANCH" == "main" || "$BRANCH" == "master" ]]; then
       if ! echo "$SUBJECT" | grep -qP '^Build \d+:'; then
           echo "WARNING: Committing to main without Build ID prefix."
           echo "Expected: 'Build N: scope: description'"
           echo "Got: '$SUBJECT'"
           echo "CHANGELOG automation will skip this commit."
           # Continue anyway — not blocking, just informational
       fi
   fi
   ```

6. **Execute the commit with Subject + Body**
   ```bash
   ...
   ```
   **Note:** The commit message MUST include both Subject and Body separated by a blank line. Never omit the Body if it exists in commit.md.
...
Commit.md entries use action verbs with `[x]` checkmarks. **ALWAYS use `- [x]` prefix** — this is non-negotiable for PR body rendering and CHANGELOG automation:

| Action Verb | When to Use | Example |
|-------------|-------------|---------|
| `Added` | New files, features, functionality | `- [x] Added user authentication` |
| `Updated` | Modifications to existing files | `- [x] Updated API endpoints` |
| `Fixed` | Bug fixes, error corrections | `- [x] Fixed login validation` |
| `Removed` | Deleted files or removed features | `- [x] Removed deprecated code` |
| `Improved` | Enhancements, better UX, performance | `- [x] Improved query performance` |
| `Changed` | Altered existing behavior | `- [x] Changed default timeout` |
...
- [x] **Category Name (N files):** Verbose description explaining purpose and function, not just file listing
- [x] **Another Category (N files):** Detailed purpose statement with "for [reason]" clarifications
```
...
|----------|-------------|---------|
| **Scripts Removal** | Deleting multiple related files | `- [x] **Linux Scripts Removal (13 files):** Removed legacy Linux-specific scripts` |
| **Hooks Added** | New hook files | `- [x] **Security Hooks Added (4 files):** Created security scanning and validation hooks` |
| **Skills Updated** | Multiple skill changes | `- [x] **Skills Updated (8 modified):** Enhanced commit, launch, openpr, repotodo skills` |
| **Config Files** | Configuration changes | `- [x] **Config Files Updated (3):** Updated settings.json, .gitignore, .claude.json` |
| **Core Scripts** | Main codebase scripts | `- [x] **Core Scripts Updated (3):** Enhanced git.py, guards.py, ralph.py` |
...
Before generating the commit message, the skill automatically encrypts `.env` files if dotenvx is configured.
...
Encryption runs **before** generating the commit message:

1. User runs `/commit`
2. Skill detects dotenvx via `package.json`
3. Runs `pnpm env:encrypt` (or `npm run env:encrypt`)
4. Stages encrypted `.env` files
5. Generates commit message in `commit.md ## Ready` with encrypted files included
...
1. **Pre-generation**: Before creating the commit message
