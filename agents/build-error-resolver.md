...
## Serena-Enhanced Error Resolution

Leverage Serena MCP tools for precise symbol location and impact analysis.

### Error Symbol Location

Use Serena to locate error sources:

```
1. Parse error message for symbol name
2. mcp__serena__find_symbol(name_path=<symbol>)
   → Get precise symbol location

3. mcp__serena__get_symbols_overview(relative_path=<file>)
   → Understand file context

4. mcp__serena__find_referencing_symbols(name_path=<symbol>)
   → Find all callers to assess impact
```

### Cascading Error Analysis

Use Serena for dependency-aware fixes:

```
1. Find root symbol causing errors
2. mcp__serena__find_referencing_symbols
   → Get all dependent symbols
3. Fix in reverse dependency order
   → Start at leaves, work to root
```

### Semantic Fix Strategy

Prefer Serena editing tools for precise fixes:

| Error Type | Serena Tool | Why |
|------------|-------------|-----|
| Type mismatch | `replace_symbol_body` | Preserves formatting |
| Missing property | `insert_after_symbol` | Adds to correct location |
| Renamed symbol | `rename_symbol` | Updates all references |
| Interface change | `replace_symbol_body` | Type-aware replacement |
...
Serena-powered resolution:

```
1. mcp__serena__find_symbol("Y")
   → Find type definition

2. mcp__serena__get_symbols_overview(relative_path=<file>)
   → Get context (is it interface, type, class?)

3. Determine if:
   - Property should be added to type Y
   - Caller should use different type

4. mcp__serena__replace_symbol_body or mcp__serena__insert_after_symbol
   → Apply fix preserving formatting
```

### Impact Assessment Before Fix

Always check impact before major changes:

```
mcp__serena__find_referencing_symbols(name_path=<symbol_to_change>)
→ If >20 references, consider backward-compatible approach
```

### Serena Memory for Build Context

Store build patterns for faster resolution:

```
mcp__serena__write_memory("build-errors", <common error patterns>)
mcp__serena__read_memory("build-errors") - Recall for similar issues
```
